...
void apex_legends(uint32_t pid) {
	...
	if (process.check_version(data.time_date_stamp, data.checksum)) {
		...
		while (process.heartbeat()) {
			...
			auto ctx = GameContext(process, data, state);
			cheats.run(ctx);
			// More? :3
		}
	}
...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		ProcessEntry entry;
		for (ProcessEnumerator enumerator{}; enumerator.next(entry); ) {
