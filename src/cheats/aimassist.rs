...
impl AimAssist {
    ...
    pub fn run(&mut self, api: &mut Api, ctx: &mut RunContext) {
        ...
        if !self
            .timer
            .has_elapsed(ctx.state.time, 1.0 / self.config.tickrate as f64)
        {
            return;
        }
    ...
    fn set_aim_mod(
        ...
        if self.config.aim_react > 0.0 {
            let react = f32::min(
                (state.time - self.target_react) as f32 / self.config.aim_react,
                1.0,
            );
        ...
        if self.config.aim_ramp > 0.0 {
            let ramp = f32::min(
                (state.time - self.target_start) as f32 / self.config.aim_ramp,
                1.0,
            );
        ...
        if self.config.trigger_again > 0.0 {
            if let Some(local) = state.local_player() {
                if let Some(weapon) = weapon {
                    let trigger = 1.0
                        - (weapon.next_ready_time - local.time_base + self.config.trigger_rdyfudge);
    ...
    fn rules<'a>(
        ...
        match target.as_ref() {
            ...
            EntityRef::Vehicle(player_vehicle) => {
                ...
                if let Some(vehicle_driver) =
                    state.entity_as::<PlayerEntity>(player_vehicle.vehicle_driver)
                {
                    if vehicle_driver.team_num == local.team_num {
                        if std::ptr::eq(vehicle_driver, local)
                            || !(self.config.aim_team || self.target_team)
                        {
                            return false;
                        }
                    }
    ...
    fn fov_check<'a>(
        ...
        let fov = if info.distance < 10.0 {
            fov_aim
        } else {
            f32::min(
                fov_aim,
                f32::atan(self.config.fov_radius / info.distance).to_degrees(),
            )
        };
...
fn is_hit(
    ...
    for i in 0..100 {
        ...
        let start = match bones.get(data[starti].0 as usize) {
            Some(start) => start,
            None => continue,
        };
        let end = match bones.get(data[endi].0 as usize) {
            Some(end) => end,
            None => continue,
        };
