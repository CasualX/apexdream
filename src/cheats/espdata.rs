...
fn draw_object(object: &Object, conf: &Config, api: &mut Api, ctx: &RunContext) {
	let alpha = if object.flags & Flags::ALPHA { object.alpha }
	else {
	...
	if object.skynade_pitch != 0.0 {
		...
		if let Some([px, py]) = ctx.angles_to_screen([-object.skynade_pitch.to_degrees(), object.skynade_yaw.to_degrees(), 0.0], false) {
			let px = px.round();
			let py = py.round();
		...
		if let Some(bones) = object.bones {
			if let Some(studio) = object.studio {
				for bbox in &studio.hitboxes {
					let matrix = match bones.get(bbox.bone as usize) { Some(matrix) => matrix, None => continue };
					...
					if let Some([x, y]) = ctx.world_to_screen(pos, true) {
						...
						if let Some([x2, y2]) = ctx.world_to_screen([pos[0], pos[1], pos[2] + radius], true) {
							let r = f32::sqrt((x - x2) * (x - x2) + (y - y2) * (y - y2));
							api.r_ellipse(x - r, y - r, r + r, r + r, vgc::sRGBA::TRANSPARENT, color);
						}
					}
				}
			}
	...
	if object.flags & Flags::BONES {
		if let Some(studio) = object.studio {
			let draw_line = |api: &mut Api, bbox: &sdk::mstudiobbox_t| {
				let Some(parent) = studio.parent_hitbox(bbox) else { return };
				let Some(bones) = object.bones else { return };
				let start = match bones.get(bbox.bone as usize) { Some(start) => start, _ => return };
				let end = match bones.get(parent.bone as usize) { Some(end) => end, _ => return };
	...
	if object.flags & Flags::BOX {
		...
		let compute_pts = || Some([
			ctx.world_to_screen(math::add(object.origin, [-hwidth, -hwidth, 0.0]), true)?,
			ctx.world_to_screen(math::add(object.origin, [-hwidth, hwidth, 0.0]), true)?,
			ctx.world_to_screen(math::add(object.origin, [hwidth, hwidth, 0.0]), true)?,
			ctx.world_to_screen(math::add(object.origin, [hwidth, -hwidth, 0.0]), true)?,
			ctx.world_to_screen(math::add(object.origin, [-hwidth, -hwidth, object.height]), true)?,
			ctx.world_to_screen(math::add(object.origin, [-hwidth, hwidth, object.height]), true)?,
			ctx.world_to_screen(math::add(object.origin, [hwidth, hwidth, object.height]), true)?,
			ctx.world_to_screen(math::add(object.origin, [hwidth, -hwidth, object.height]), true)?,
		]);
	...
	if (object.flags & Flags::AIM) && object.visible {
		if let Some(aim) = object.aim {
			if let Some([x, y]) = ctx.angles_to_screen(aim, true) {
				let x = x.round();
				let y = y.round();
				api.r_rect(/*x:*/ x - 4.0, /*y:*/ y - 1.0, /*width:*/ 9.0, /*height:*/ 3.0, /*fill:*/ dark, /*stroke:*/ vgc::sRGBA::TRANSPARENT);
				api.r_rect(/*x:*/ x - 1.0, /*y:*/ y - 4.0, /*width:*/ 3.0, /*height:*/ 9.0, /*fill:*/ dark, /*stroke:*/ vgc::sRGBA::TRANSPARENT);
	...
	let Some(bounds) = bounds(object, conf, ctx) else { return };

	if object.flags & Flags::TEXT {
		if let Some(text) = if conf.debug_models { object.model_name } else { object.text } {
			if let Some([x, y]) = ctx.world_to_screen(object.origin, true) {
				api.r_text(
					/*font:*/ 0, /*flags:*/ 3,
					x, y,
	...
	if object.flags & Flags::BOUNDS {
		...
		if object.visible {
			api.r_rect(
				...
				/*y:*/ bounds.top + 1.0,
				width, height,
				/*fill:*/ vgc::sRGBA::TRANSPARENT, /*stroke:*/ dark,
			);
			api.r_rect(
				/*x:*/ bounds.left,
				/*y:*/ bounds.top,
				width, height,
				/*fill:*/ vgc::sRGBA::TRANSPARENT, /*stroke:*/ color,
			);
		}
		else {
...
fn bounds(object: &Object, conf: &Config, ctx: &RunContext) -> Option<ObjectBounds> {
	...
	let vtop = [object.origin[0], object.origin[1], object.origin[2] + object.height];
	...
	let [cx, cy] = ctx.world_to_screen([object.origin[0], object.origin[1], object.origin[2] + object.height * 0.5], true)?;
