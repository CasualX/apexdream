...
impl Debugger {
	pub fn run(&mut self, api: &mut Api, ctx: &mut RunContext) {
		...
		if self.timer.has_elapsed(ctx.state.time, 1.0/20.0) && (self.config.enable || ctx.state.is_firing_range()) {
			self.state(api, ctx);
			self.world(api, ctx);
			self.player(api, ctx);
			self.loot(api, ctx);
	...
	fn world(&mut self, api: &mut Api, ctx: &RunContext) {
		...
		let Some(world) = state.world_entity() else { return };
	...
	fn script_data(&mut self, api: &mut Api, ctx: &RunContext) {
		...
		api.visualize(s!("ScriptData"), xfmt! {
			<pre><table>
			<tr>
				<td>"Index"</td>
				<td>"Name"</td>
	...
	fn scriptnetdata(&self, api: &mut Api, ctx: &RunContext) {
		let state = ctx.state;
		let Some(local) = state.local_player() else { return };
		let Some(global) = state.entity_as::<ScriptNetDataEntity>(local.script_net_data_global) else { return };
		let Some(exclusive) = state.entity_as::<ScriptNetDataEntity>(local.script_net_data_exclusive) else { return };
		api.visualize(s!("ScriptNetData"), xfmt! {
			<h1>"Global"</h1>
			<pre>
				("Bools: "{global.bools():?}"\n")
...
fn print_weapon_mods(f: &mut fmt::Formatter, state: &GameState, weapon_name: sdk::WeaponName, mod_bitfield: u32) -> fmt::Result {
	fmtools::write!(f,
