...
impl Projectile {
    pub fn run(&mut self, api: &mut Api, ctx: &mut RunContext) {
        ...
        if self.in_attack != in_attack
            && in_attack != state.weapon_is_charged(weapon.weapon_name_index)
        {
        ...
        if !self.handle.is_valid() {
            if let Some(p) = state
                .entities_as::<ProjectileEntity>()
                .filter(|p| p.owner_entity == state.client.local_entity)
                .next()
            {
                ...
                if self.launch_origin != p.launch_origin {
                    ...
                    let dx =
                        f32::sqrt(p.velocity[0] * p.velocity[0] + p.velocity[1] * p.velocity[1]);
