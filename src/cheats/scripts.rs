...
impl Scripts {
    ...
    fn autoreload(&mut self, ctx: &mut RunContext) {
        ...
        let spare_ammo =
            local.get_consumable_count(state, weapon.ammo_item(state)) + weapon.ammo_in_stockpile;
        if spare_ammo <= 0 {
            return;
        }
    ...
    fn tapstrafe(&mut self, ctx: &mut RunContext) {
        ...
        if is_on_ground {
            ...
            if state.is_button_down(self.config.tapstrafekey)
                && (state.in_moveleft() || state.in_moveright())
            {
                ctx.forward = if ctx.rapidfire() {
                    InState::Press
                } else {
                    InState::Release
                };
                ...
                if let Some(local) = ctx.state.local_player() {
                    if ctx.entity_check(api, local.index, local.entity_ptr) {
                        let _ = api.vm_write(
                            local
                                .entity_ptr
                                .field(ctx.data.player_third_person_shoulder_view),
                            if is_ads { &0i32 } else { &1i32 },
                        );
                    }
                }
            }
    ...
    fn freecam(&mut self, api: &mut Api, ctx: &mut RunContext) {
        ...
        if ctx.entity_check(api, player.index, player.entity_ptr) {
            let _ = api.vm_write(
                player.entity_ptr.field(ctx.data.player_observer_state),
                &mode,
            );
        }
    }
