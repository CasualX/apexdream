...
impl std::str::FromStr for Flags {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		for s in s.split_ascii_whitespace() {
			if s == s!("none") { flags |= Flags::NONE.0; }
			else if s == s!("bounds") { flags |= Flags::BOUNDS.0 }
			else if s == s!("box") { flags |= Flags::BOX.0 }
			else if s == s!("text") { flags |= Flags::TEXT.0 }
			else if s == s!("name") { flags |= Flags::NAME.0 }
			else if s == s!("health") { flags |= Flags::HEALTH.0 }
			else if s == s!("icon") { flags |= Flags::ICON.0 }
			else if s == s!("bones") { flags |= Flags::BONES.0 }
			else if s == s!("trail") { flags |= Flags::TRAIL.0 }
			else if s == s!("spine") { flags |= Flags::SPINE.0 }
			else if s == s!("barrel") { flags |= Flags::BARREL.0 }
			else if s == s!("aim") { flags |= Flags::AIM.0 }
			else if s == s!("faded") { flags |= Flags::FADED.0 }
			else if s == s!("alpha") { flags |= Flags::ALPHA.0 }
			else if s == s!("origin") { flags |= Flags::ORIGIN.0 }
			else if s == s!("skydot") { flags |= Flags::SKYDOT.0 }
			else {
