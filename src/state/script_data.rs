...
impl ScriptValue {
	...
	pub const fn to_bool(self) -> Option<bool> {
		match self { ScriptValue::Bool(value) => Some(value), _ => None }
	}
	#[inline]
	pub const fn to_byte(self) -> Option<u8> {
		match self { ScriptValue::Byte(value) => Some(value), _ => None }
	}
	#[inline]
	pub const fn to_word(self) -> Option<i16> {
		match self { ScriptValue::Word(value) => Some(value), _ => None }
	}
	#[inline]
	pub const fn to_int(self) -> Option<i32> {
		match self { ScriptValue::Int(value) => Some(value), _ => None }
	}
	#[inline]
	pub const fn to_float(self) -> Option<f32> {
		match self { ScriptValue::Float(value) => Some(value), _ => None }
	}
	#[inline]
	pub const fn to_time(self) -> Option<f32> {
		match self { ScriptValue::Time(value) => Some(value), _ => None }
	}
}
...
impl ScriptNetData {
	pub fn update(&mut self, api: &mut Api, ctx: &UpdateContext) {
		...
		if ctx.connected {
			let table_ptr = ctx.process.base.field::<[NetVarEntry]>(ctx.data.network_var_table_ptr);
...
impl GameState {
	pub fn read_script_value(&self, name: sdk::ScriptNetVarName, index: sdk::EHandle) -> ScriptValue {
		...
		let entry = loop {
			let Some(entry) = entries.get(name_index) else { return ScriptValue::default() };
		...
		match entry.value_type {
			...
			2 => {
				script_net_data.ranges
					.get(entry.value_index as usize)
					.map(|&value| ScriptValue::Word(value))
					.unwrap_or(ScriptValue::Invalid)
			},
			3 => {
				script_net_data.ints
					.get(entry.value_index as usize)
					.map(|&value| ScriptValue::Int(value))
					.unwrap_or(ScriptValue::Invalid)
			},
			5 => {
				script_net_data.times
					.get(entry.value_index as usize)
