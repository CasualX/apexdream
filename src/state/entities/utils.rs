...
impl BoneArray {
	pub fn update(&mut self, api: &mut Api, ctx: &UpdateContext, studio: &StudioModel, ptr: sdk::Ptr<[[f32; 12]]>) {
		if ptr.is_null() {
			self.v.clear();
			return;
		}
		let numbones = if ctx.full_bones { studio.bone_end2 } else { studio.bone_end1 } as usize;
