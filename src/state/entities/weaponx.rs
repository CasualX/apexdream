...
impl WeaponXEntity {
	pub fn new(entity_ptr: sdk::Ptr, index: u32, cc: &sdk::ClientClass) -> Box<dyn Entity> {
		...
		Box::new(WeaponXEntity { entity_ptr, entity_size, index, ..WeaponXEntity::default() }) as Box<dyn Entity>
	}
	...
	pub fn desired_items(&self, state: &GameState) -> sdk::ItemSet {
		let Some(mods) = state.get_mods(self.weapon_name) else { return Default::default() };
	...
	fn update(&mut self, api: &mut Api, ctx: &UpdateContext) {
		...
		let mut indices = Indices {
			...
			weapon_name_index: [
				...
				data.weaponx_weapon_name_index + 0x14,
		...
		if let Ok(fields) = api.vm_gatherd(self.entity_ptr, self.entity_size, &mut indices) {
			...
			self.modifiers_ptr = sdk::Ptr::from_raw(fields.weapon_name_index[1] as u64 | (fields.weapon_name_index[2] as u64) << 32);

			self.mod_bitfield = fields.mod_bitfield[0] | fields.mod_bitfield[1] | fields.mod_bitfield[2];

			self.is_semi_auto = fields.data[0].to_le_bytes()[data.weaponx_is_semi_auto as usize & 3] != 0;
			self.projectile_speed = f32::from_bits(fields.projectile[0]);
			self.projectile_scale = f32::from_bits(fields.projectile[1]);
		}

		self.update_rate = if self.weapon_owner == ctx.local_entity { 1 } else { 512 };
	}
...
impl crate::base::solver::ProjectileWeapon for WeaponXEntity {
	...
	fn projectile_gravity(&self) -> f32 {
		/*sv_gravity*/750.0 * self.projectile_scale
	}
