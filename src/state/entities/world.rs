...
impl DeathField {
	pub fn radius(&self, curtime: f32) -> f32 {
		...
		let fraction = ((curtime - self.time_start) / (self.time_end - self.time_start)).max(0.0).min(1.0);
...
impl WorldEntity {
	pub fn new(entity_ptr: sdk::Ptr, index: u32, cc: &sdk::ClientClass) -> Box<dyn Entity> {
		...
		Box::new(WorldEntity { entity_ptr, entity_size, index, ..WorldEntity::default() }) as Box<dyn Entity>
	}
}
