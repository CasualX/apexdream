...
impl PlayerEntity {
    ...
    pub fn desired_items(&self, state: Option<&GameState>) -> sdk::ItemSet {
        ...
        let mut set = bitset! {
            ...
            sdk::ItemId::SkullpiercerRifling.0,
            sdk::ItemId::Turbocharger.0,
    ...
    pub fn accel_angle(&self) -> f32 {
        let Some((_, olddir)) = self.derivative_velocity.history.first() else {
            return 0.0;
        };
    ...
    fn update(&mut self, api: &mut Api, ctx: &UpdateContext) {
        ...
        if let Ok(fields) = api.vm_gatherd(self.entity_ptr, self.entity_size, &mut indices) {
            ...
            if ctx.time >= self.trail_next {
                ...
                self.trail_next = if self.trail_next + 0.2 > ctx.time {
                    self.trail_next + 0.2
                } else {
                    ctx.time + 0.2
                };
            ...
            let team_color = if self.team_num < 1 {
                [255, 255, 255]
            } else {
                TEAM_COLORS[(self.team_num - 1) as usize % TEAM_COLORS.len()]
            };
