...
impl ClientState {
    pub fn update(&mut self, api: &mut Api, ctx: &mut UpdateContext) {
        ...
        if ctx.ticked(25, 24) {
            if let Ok(signon_state) = api.vm_read::<i32>(
                process
                    .base
                    .field(data.client_state + data.client_signon_state),
            ) {
        ...
        if ctx.connected {
            ...
            if let Ok(level_name) = api.vm_read_cstr(
                process
                    .base
                    .field(data.client_state + data.client_level_name),
                &mut level_name,
            ) {
        ...
        if ctx.connected || ctx.ticked(100, 11) {
            let _ = api.vm_read_into(
                process.base.field(data.local_entity_handle),
