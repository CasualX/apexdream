...
pub fn load_string_table(st: &mut Box<[String]>, api: &mut Api, ctx: &UpdateContext, offset: u32) -> Result<(), api::Error> {
...
impl StringTables {
	...
	pub fn debug(&self, api: &mut Api) {
		api.visualize(s!("StringTables"), xfmt! {
			<h2>"Weapon Names"</h2>
			<pre>
...
impl GameState {
	pub fn weapon_string(&self, weapon_name_index: i32) -> Option<&str> {
		Some(&*self.string_tables.weapon_names.get(weapon_name_index as usize)?)
	}
	pub fn weapon_name(&self, weapon_name_index: i32) -> sdk::WeaponName {
		let Some(weapon_name) = self.string_tables.weapon_names.get(weapon_name_index as usize) else { return Default::default() };
