    ...
    pub fn update(&mut self, api: &mut Api, ctx: &UpdateContext) {
        ...
        if let Some(ent_info_slice) = self.ent_info.get_mut(start..end) {
            let _ = api.vm_read_into(
                process.base.field(ctx.data.entity_list + start as u32 * 32),
                ent_info_slice,
            );
        }
...
impl GetClientEntity {
    ...
    fn create_entity(
        ...
        match data.name_hash {
            sdk::CPlayer => Some(PlayerEntity::new(entity_ptr, index, &data.client_class)),
    ...
    fn get_client_class(
        ...
        if !self.lookup.contains_key(&client_networkable) {
            ...
            if get_client_entity.into_raw() == 0xFFFFFFFFFFFFFFFF {
                return None;
            }
