...
impl GameProcess {
	...
	pub fn is_ready(&mut self, api: &mut Api, time: f64) -> bool {
		...
		if api.vm_read_into(base.field(dos_header.e_lfanew), &mut nt_headers).is_err() {
			return false;
		}

		let mut text_section: image::IMAGE_SECTION_HEADER = dataview::zeroed();
		let section_headers = dos_header.e_lfanew + size_of::<u32>() as u32 + size_of::<image::IMAGE_FILE_HEADER>() as u32 + nt_headers.FileHeader.SizeOfOptionalHeader as u32;
		if api.vm_read_into(base.field(section_headers), &mut text_section).is_err() {
			return false;
		}

		let mut bytes = [0u8; 8];
		if api.vm_read_into(base.field(text_section.VirtualAddress + text_section.SizeOfRawData - 8), &mut bytes).is_err() {
		...
		if n_pad > 0 {
			if if n_pad >= 8 { bytes != [0u8; 8] } else { bytes[7] != 0 } {
				return false;
			}
		}
	...
	pub fn read_pages(&mut self, api: &mut Api, time: f64) {
		...
		let interval = if time < self.dump_start + 5.0 { 0.2 }
		else { 1.0 / self.dump_rate as f64 };
		if !self.dump_timer.has_elapsed(time, interval) {
			return;
		}

		let offset = (self.page_index * 0x1000) as usize;
		let Some(image_page) = self.image_data.get_mut(offset..offset + 0x1000) else { return };
		if api.vm_read_into(self.base.field(offset as u32), image_page).is_err() {
			// Reading can fail in which case we just need to wait until the game touches the page
			self.read_errors += 1;
			let _ = api.vm_write(self.base.field(self.gadgets[0] + 0x48), &self.base.field::<u8>(self.page_index * 0x1000));
			return;
		}
		...
		if self.page_index == num_pages {
			self.dump_bin(api);
			// Restore the gadgets
			let _ = api.vm_write(self.base.field(self.gadgets[0] + 0x48), &self.base.field::<u8>(self.gadgets[0]));
			let _ = api.vm_write(self.base.field(self.gadgets[1] + 0x48), &self.base.field::<u8>(self.gadgets[1]));
			// Free memory
			self.dump_game = false;
			self.image_data = Box::default();
		}
			...
			let _ = api.vm_write(self.base.field(self.gadgets[0] + 0x48), &self.base.field::<u8>(self.page_index * 0x1000));
			let _ = api.vm_write(self.base.field(self.gadgets[1] + 0x48), &self.base.field::<u8>(u32::min(self.page_index + 1, num_pages - 1) * 0x1000));
		}
	}
	...
	fn find_gadget(&mut self, api: &mut Api) -> bool {
		...
		while i < self.size_of_image {
			...
			let Some(ptr) = image.try_read::<sdk::Ptr>(rva as usize) else { continue };
			...
			if image.try_get::<[u8; 11]>(offset as usize) == Some(cl_showpos) {
				...
				if image.try_read::<u64>((rva + 6*8) as usize) != Some(self.base.field::<()>(gadget_rva).into_raw()) {
			...
			// Find fps_max
				...
				// Double check the gadget before using it
				if image.try_read::<u64>((rva + 6*8) as usize) != Some(self.base.field::<()>(fps_max).into_raw()) {
	...
	fn dump_bin(&mut self, api: &mut Api) {
		...
		for section in section_headers {
			...
			if &section.Name == b".reloc\0\0" {
				if let Some(reloc_dir) = data_directory.get_mut(pelite::image::IMAGE_DIRECTORY_ENTRY_BASERELOC) {
